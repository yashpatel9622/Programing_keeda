...
#left-panel{width:22%;height:3620px;background-color:#eda150;float:left;padding:10px;}
		.sidebar ul
		{
			list-style:none;
			margin:0;
...
#right-section{width:73%;height:3600px;background-color:#bcc92b;float:right;padding:20px;font-size:28px;}
.holder1{
padding:10px;
font-size:25px;
}
...
<body>
	<div id="wrapper">
		...
		<div id="container">
			...
			<div id="right-section">	
				...
				<div class="holder1"><h1><center><i><b><p>Defination, Declaration and calling function</p></b></i></center></h1></div>	
				
				<div class="holder3">
		
					<ul>
						<li>Function declaration is done to tell the compiler about the existance of the function.Function's return type, its name and parameter list is mentioned.Function body is written in its definition.</li>
						<br>
	...
	//defining functionint sum(int x,int y)
	{
		return(x+y);
	}
					...
					<div class="holder3">
					<li>Here, initially the function is declares, without body.Then inside main() function it is called, as the function returns sumation of two values, hence z is their to store the value of sum. Then, at last, function is defined, where the body of function is mentioned. We can also, declare and define the function together, but then it should be done before it is called.<li>
					<br>
					<br>
					<h4 style=color:green>Calling a Function</h4>
					<li>Functions are called by their names.If the function is without argument, it can be called directly using its name.But for functions with arguments,we have two ways to call them,</li>
					<h5>1.Call by Value</h5>
					<h5>2.Call by Reference</h5>
					<br>
					<h4 style=color:green>Calling by Value</h4>
					<li>In this calling technique we pass the values of arguments which are stored or copied into the formal parameters of functions.Hence, the original values are unchanged only the parameters inside function changes.</li>
					<br>
	...
	void calc(int x)
	{
		x=x+10;
					...
					<p>10</p>
					<br>
					<p>In this case the actual variable x is not changed, because we pass argument by value, hence a copy of x is passed which is changed, and that copied value is destroyed as the function ends(goes out of scope).So the variable x inside main() still has a value 10.</p>
					<br>
					<p>But we can change this program to modify the original x, by making function calc() return a value, and storing that value in x.
					<br>
					<pre>
	int calc(int x);
	int main()
	...
	int calc(int x)
	{
		...
		return x;
					...
					<br>
					<p>20</p>
					<h4 style=color:green>Call by Reference</h4>
					...
					<br>
					<h4 style=color:green>Call by Example</h4>
					<br>
					<pre>
	void calc(int *p);
	int main()
	...
	void calc(int *p)
	{
		*p=*p+10;
				...
				<div class="but">
				<a href="cpp_func.php" class="button prev">&laquo;prev</a>
				<a href="cpp_in.php" class="button next">next &raquo;</a>
				</div>
			</div>
		</div>
		<div id="footer">
