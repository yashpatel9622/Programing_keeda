...
<i>Description</i>
<p>
 Generates a signal: Sends signal to the current executing program. </br>The signal is handled as specified by function signal 

</p>
<i>prototype</i>
<b>  int raise (int sig); </b>
<i>parameters</i>
<p>
 Parameters sig The signal value to raise.</br> The following macro constant expressions identify standard signal values: </br>macro signal SIGABRT (Signal Abort) Abnormal termination,</br> such as is initiated by the abort function. SIGFPE (Signal Floating-Point Exception)</br> Erroneous arithmetic operation, such as zero division or an operation resulting in overflow </br>(not necessarily with a floating-point operation). (Signal Illegal Instruction) </br>Invalid function image, such as an illegal instruction. This is generally due </br>to correct in the code or to an attempt to 
Executedata SIGINT (signal interrupt interactive attension signal.
